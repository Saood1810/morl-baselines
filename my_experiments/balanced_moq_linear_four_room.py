...
for seed in SEEDS:
  ...
  exp_name = f" Exploration-Exploitation MOQ Linear Experiment in RG with seed Gamma 0.9 {seed}"
  ...
  for i in range(0, len(weight_combinations)):
    ...
    agent = MOQLearning(env, scalarization=weighted_sum,initial_epsilon=1,final_epsilon=0.1,epsilon_decay_steps=800000, gamma=0.9, weights=weights, log=False)
  ...
  log_unknown_results(pf,hypervolume_scores,cardinality_scores,sparsity_scores,"Research Project Logs V6",exp_name,"Exploration-Exploitation MOQ Linear Four Room")
