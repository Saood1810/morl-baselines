...
for seed in SEEDS:
  ...
  exp_name = f"Greedy Chebyshev Experiment with seed {seed}"
  ...
  for i in range(0, 11):
      ...
      linear = MOQLearning(env, scalarization=tchebicheff(tau=4.0, reward_dim=2),initial_epsilon=0.1,final_epsilon=0.1, gamma=0.99, weights=weights,seed=seed, log=False)
  ...
  log_results(pf,hypervolume_scores,cardinality_scores,igd_scores,sparsity_scores,"Greedy MOQ Chebyshev Scalarized DST ",exp_name,"Greedy MOQ Chebyshev DST")
  
  print("Greedy MOQ Chebyshev Results for seed: ",seed)
  print(pf)
