...
for seed in SEEDS:
  ...
  exp_name = f"0.05 Cheby Experiment G 0.9 with seed {seed}"
  ...
  for i in range(0, 11):
      ...
      chebyshev = MOQLearning(env, scalarization=scalarization,initial_epsilon=1,final_epsilon=0.1,epsilon_decay_steps=0.05*400000, gamma=0.9, weights=weights,seed=seed, log=False)
  ...
  log_results(pf,hypervolume_scores,cardinality_scores,igd_scores,sparsity_scores,"Research Project Logs V4",exp_name,"MOQ Cheb DST with gamma of 0.9")
